...
const HomeClient = props => {
  ...
  const submitDelete = async () => {
    ...
    if (res.data) {
      let new_users = clients.filter(function (deliveryman) {
        return deliveryman.id !== deliveryman_id;
      });

      props.navigation.navigate('Home');
      setCllients(new_users);
    }
  ...
  const search = searchText => {
    ...
    let filteredData = clients.filter(function (item) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        data={filteredData && filteredData.length > 0 ? filteredData : clients}
